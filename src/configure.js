...
class Lib {
  getSourceFiles() {
    ...
    if (v === undefined) {
      this[kExpandedSources] = v = globv(this.sources.map(fn =>
        Path.resolve(this[kConfig].projectdir, fn))
      )
    }
    return v
  }
...
rule emccobj
  ...
  for (let m of config.modules) {
    ...
    for (let depname of m.deps) {
      ...
      if (!depobjfiles) {
        throw new Error(`wasm module ${m.name} specifies undefined dep: ${depname}`)
      }
